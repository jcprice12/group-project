...
function getCard(title, servings, time, img, url, recipeId, likes) {
  let card = `
    <my-card
      url="${url}"
      source="${img}"
...
function cardsEventApi() {
  $('body').on('click', '#search', (e) => {
    ...
    // obj.excludeIngredients = $('#exclude-ingredients').val() && $('#exclude-ingredients').val();
    // obj.maxCalories = $('#max-calories').val() && $('#max-calories').val();
    // obj.minCalories = $('#min-calories').val() && $('#min-calories').val();
    // obj.diet = $(".diet:checked").attr("id") && $(".diet:checked").attr("id");
    // obj.intolerances = allIntolerances.length > 0 && allIntolerances;
...
function searchRecipes(url, config) {
  ...
  // let promises = axios.get(url, config)
  //   .then((res) => {
...
function getRecipe(url, config, state, recipeId) {
  ...
  axios.get(url, config)
    .then((res) => {
      let id = res.data.id;
      axios.get('https://project1-4f221.firebaseio.com/recipes/' + id + '.json')
        .then((res) => {
          console.log(res);
        })
      let tempState = {};
